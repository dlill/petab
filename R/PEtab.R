...
petab_create_parameter_df <- function(pe, observableParameterScale = "log10") {
  ...
  pfl                   <- pe$model$parameterFormulaList
...
petabYaml <- function(parameter_file = NULL, 
  ...
  yaml_content <- list(format_version = 1, # Update if necessary
                       ...
                       problems = list(list( # One problem
                         ...
                         observable_files = list(observable_files), 
                         sbml_files = list(sbml_files), 
                         visualization_files = list(visualization_files))), 
                       meta_file = meta_file, 
...
petab_lint <- function(pe) {
  ...
  if (!is.null(pe$observables) & !is.null(pe$measurementData)) {
    ...
    dco[,`:=`(nobspars = do.call(c,lapply(observableFormula,   function(x) {max(as.numeric(stringr::str_extract_all(stringr::str_extract_all(x, "observableParameter\\d+", simplify = T), "\\d+", simplify = T)))})))]
    ...
    dco[,`:=`(nobspars = do.call(c,lapply(noiseFormula,   function(x) max(as.numeric(stringr::str_extract_all(stringr::str_extract_all(x, "noiseParameter\\d+", simplify = T), "\\d+", simplify = T))))))]
