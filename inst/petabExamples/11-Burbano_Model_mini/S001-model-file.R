...
save(x, g, p0, file = "001-model.rds")
unlink(list.files(pattern = "(\\.o|\\.c)$"))
loadDLL(x)
setwd(.currentWD)
# -------------------------------------------------------------------------#
# Fit model ----
...
if(FALSE){
  ...
  fitvalue <- round(myfitlist[1]$value, digits = 2)
  print(fitvalue)
  saveRDS(myfitlist, "myframe.rds")
  saveRDS(bestfit, "bestfit_-102.rds")
...
# .. 1 load bestfit -----
myframe <- readRDS(file = "myframe.rds")
step <- 1
...
# .. Write petab -----
filename <- "Model"
writePetab(pe, file.path("05-PEtab",filename))

unlink(list.files(".", "\\.o$|\\.so$|\\.c$"))

# .. Test petab by import and obj evaluation -----

importPEtabSBML(modelname = "Model", path2model = "05-PEtab/")
